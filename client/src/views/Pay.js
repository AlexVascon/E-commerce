...
import { View, Error, Row, RowText } from '../components/View'
...
export default function Pay() {
  ...
  const {payOrderError, payOrderSuccess, payOrderLoading} = useSelector((state) => state.payOrder)
    ...
    return (
      <PaymentContainer onSubmit={handleCheckoutSubmit}>
          ...
          <Row>
            ...
            <RowText>${props?.totalPrice && props.totalPrice + props?.taxPrice}</RowText>
          </Row>
  ...
  return (
    <View>
      ...
      <Disclaimer>Warning: Stripe payment is in test mode. None of the products are real. This is merely a project. If you wish to proceed simply type 4242 etc over and over in that order. I plan to add to this.</Disclaimer>
      {payOrderLoading && <ProcessingPayment>
        <RowText>Proccessing payment. This will take a moment.</RowText>
        <ProcessingSpinner />
      </ProcessingPayment>}
     {foundOrder && <PaymentForm totalPrice={foundOrder.totalPrice} taxPrice={foundOrder.taxPrice} />} 
      </Elements>
      <Dialog open={open} onClose={closeSuccessScreen}>
      <SuccessScreen>Payment Success. Click anywhere to return to account.</SuccessScreen>
      </Dialog>
    </View>
  )
}
