...
export const createUser = async (req, res, next) => {
  try {
    ...
    if (password !== confirmPassword)
      return res.status(400).send({ messages: 'passwords dont match.' })
    ...
    await cart.save()
    if (cartItems) {
      // if user added cart items without account
      await Promise.all(
...
export const fetchUser = async (req, res, next) => {
  try {
    ...
    if (!usernameOrEmail || !password)
      return res.status(404).send({ messages: 'username or password invalid' })
    const searchCriteria = (await usernameOrEmail.includes('@'))
      ? 'email'
      : 'username'
    const user = await User.findOne({ [searchCriteria]: usernameOrEmail })
      .lean()
      .orFail()
...
export const fetchAllUsers = async (req, res, next) => {
  try {
    ...
    res.status(200).send({ users, pageCount, page })
  } catch (err) {
    next(err)
  }
}
  ...
  try {
    ...
    if (shipping) {
      shipping.information = {
        ...
        streetAddress:
          req.body.streetAddress || shipping.information.streetAddress,
      }
      await shipping.save()
      return res.status(201).send({ message: 'updated shipping information.' })
    }
    ...
    await shipping.save()
    res.status(201).send({ message: 'saved shipping information.' })
  } catch (err) {
    next(err)
  }
}
...
export const fetchShippingInformation = async (req, res, next) => {
  try {
    const shipping = await Shipping.findOne({ userId: req.user._id }).orFail()
